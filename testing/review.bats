...
# Based on https://raw.githubusercontent.com/CircleCI-Public/orb-tools-orb/refs/heads/master/src/scripts/review.bats
...
@test "RC004: Usage example names should be descriptive." {
    ...
    for i in $(find "${ORB_SOURCE_DIR}/examples/*.yml" -type f 2>/dev/null); do
        if [[ $i =~ "example" ]]; then
            echo
...
@test "RC009: Complex Run step's commands should be imported." {
    ...
    ERROR_COUNT=0
    ORB_VAL_MAX_COMMAND_LENGTH=${ORB_VAL_MAX_COMMAND_LENGTH:-150}
    for i in $(find ${ORB_SOURCE_DIR}/jobs ${ORB_SOURCE_DIR}/commands -name "*.yml" 2>/dev/null); do
        ORB_COMPONENT_STEPS_COUNT=$(yq -o json '.' "$i" | jq '[. | paths ] | map(select(last=="run")) | length')
        RUN_ENTRIES=$(yq -o json '.' "$i" | jq -r '[paths] | map(select(last == "run") | join(".")) | .[]')
        for ENTRY in $(echo "$RUN_ENTRIES"); do
            ORB_COMPONENT_STEP_COMMAND=$(yq ".$ENTRY.command" "$i")
            ORB_COMPONENT_LINE_NUMBER=$(yq ".${ENTRY}.command | line" "$i")
            if [[ ${#ORB_COMPONENT_STEP_COMMAND} -gt ${ORB_VAL_MAX_COMMAND_LENGTH} ]] && [[ ! "$ORB_COMPONENT_STEP_COMMAND" =~ \<\<include\(* ]]; then
                echo "File: \"${i}\""
                echo "Line number: ${ORB_COMPONENT_LINE_NUMBER}"
    ...
    if [[ "$ERROR_COUNT" -gt 0 ]]; then
        ...
        echo "Writing commands externally will allow you to take advantage of syntax highlighting and avoid mixing code and markup."
